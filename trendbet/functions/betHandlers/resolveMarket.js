...
const resolveMarket = functions.https.onCall(async (data, context) => {
  ...
  try {
    await AdminValidator.validateAdmin(context.auth.uid);
  } catch (error) {
  ...
  try {
    ...
    Object.keys(bets).forEach((betId) => {
      ...
      if (bet.outcome === result) {
        ...
        const payout = OddsCalculator.calculatePayout(bet.amount, bet.odds);
        ...
        updates[userPath].balance = admin.database.ServerValue.increment(payout);
